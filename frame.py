...
class ImageFrame(customtkinter.CTkFrame):
    ...
    def __init__(self, master) -> None:
        ...
        self.label.grid(row=1, column=0, padx=0, pady=(0, 0))

class SelectWidgetFrame(customtkinter.CTkFrame):
    def __init__(self, master, title: str, values: list[str]) -> None:
        super().__init__(master)
        self.title = title
        self.values = values
        self.title = customtkinter.CTkLabel(self, text=self.title, fg_color="transparent")
...
class RunButtonFrame(customtkinter.CTkFrame):
    def __init__(self, master, title: str, command) -> None:
        ...
        self.button_img = Image.open(r"image/light.png")
        ...
        self.select.grid(row=3, column=0, padx=80, pady=(0, 20), sticky="w")
